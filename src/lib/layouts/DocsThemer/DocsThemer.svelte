<script lang="ts">
    ...
    function generateColorCssJSON(): string {
		let newCssInJs = {};
		const newPalette: Record<string, Palette> = {};
		// Loop store colors
		$storeThemGenForm.colors.forEach((color: ColorSettings, i: number) => {
...
<div class="docs-themer space-y-4">
	...
	<div class="grid grid-cols-2 gap-4">
		...
		<footer class="col-span-2 space-y-4">
			...
			<div class="card variant-glass p-4 text-center flex items-center justify-center gap-4">
				<!-- prettier-ignore -->
				<button class="btn btn-lg variant-filled-primary font-bold" on:click={() => {
                    axios.post(`${config.apiEndpoint}/set-profile-css`, {
                        css: cssOutput
                    }, {
                        headers: {
                ...
                <button class="btn btn-lg variant-filled-error font-bold" on:click={() => {
                    axios.post(`${config.apiEndpoint}/set-profile-css`, {
                        css: "/* No theme set */"
                    }, {
                        headers: {
