<script lang="ts">
    ...
    axios.get(`${config.apiEndpoint}/message/${message}`, {
        ...
        let otherMessage = await axios.get(`${config.apiEndpoint}/message/${res.data.parent}`, {
            headers: {
                Authorization: localStorage.getItem('sessionToken')
            }
        })
        subject.set(otherMessage.data.subject)
        user.set(profile.data.userData);
    })
</script>
{#if $messageData && $user}
    <div class="card variant-filled-surface w-full p-4 min-w-56">
        <p class="font-bold">New reply to "{$subject}"</p>
        ...
        <div class="flex gap-2">
            <button class="btn variant-filled-primary btn-sm" on:click={()=>{
                window.location = `/messages?id=${$messageData.parent}`
            }}>
                View
            </button>
