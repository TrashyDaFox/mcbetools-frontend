...
<a data-sveltekit-reload={true} id={project.url} key={project.url} href={preventClick ? "#" : edit ? null : `/s/${isDraft ? "draft-" : ""}${project.url}`} class="{extraClasses} {f1 ? "!flex-auto " : ""}{`mt-4 card bg-initial to-surface-700 card-hover md:w-fit rounded-lg overflow-hidden${!f1 ? " w-96 sm:w-full " : " "}flex flex-col`} {!f1 ? "min-w-full" : ""} {project.specialTags.includes('WOMEN_ONLY') && isDark ? "!border-primary-500 !border-2 !border-dashed !bg-gradient-to-br from-primary-900/70 to-primary-500/10 !rounded-3xl" : ""} max-w-0" on:dragstart|preventDefault style={extraStyles} data-theme={project.specialTags && project.specialTags.includes('WOMEN_ONLY') ? "cherry" : ""} class:featured={project.featured && ![1, 2, 3].includes(project.featureLevel)} class:legendary={project.featured && project.featureLevel == 1} class:mythic={project.featured && project.featureLevel == 2} class:plasmatic={project.featured && project.featureLevel == 3} on:click={onClick}>
