...
<a data-sveltekit-reload={true} id={project.url} key={project.url} href={preventClick ? "#" : edit ? null : `/s/${isDraft ? "draft-" : ""}${project.url}`} class="relative {extraClasses} {f1 ? "!flex-auto " : ""}{`mt-4 card bg-initial to-surface-700 card-hover md:w-fit rounded-lg overflow-hidden${!f1 ? " w-96 sm:w-full " : " "}flex flex-col`} {!f1 ? "min-w-full" : ""} {project.specialTags.includes('WOMEN_ONLY') && isDark ? "!border-primary-500 !border-2 !border-dashed !bg-gradient-to-br from-primary-900/70 to-primary-500/10 !rounded-3xl" : ""} max-w-0" on:dragstart|preventDefault style={extraStyles} data-theme={project.specialTags && project.specialTags.includes('WOMEN_ONLY') ? "cherry" : ""} class:featured={project.featured && ![1, 2, 3, 4].includes(project.featureLevel)} class:legendary={project.featured && project.featureLevel == 1} class:mythic={project.featured && project.featureLevel == 2} class:plasmatic={project.featured && project.featureLevel == 3} class:celestial={project.featured && project.featureLevel == 4} on:click={onClick}>
    <!-- <div class="absolute w-full h-full z-1000 top-0 left-0"> -->
    <!-- <Particle particleString="30a-1a1.3a0.2a20a90a0a10a5a20a20a0a0a8a0a0a0a4a1a0a0a0a0a1a0a1a0a1a0a1a1a0a0a0 a0a0.784314a0a1a0a1a0a0.27a0a0.27a0a0a0a0a0a0a0a0a2a1a0a0a0a0a0a0a0.25a0a0a0a0a0a0a0a0a0a0a0" /> -->
    ...
    <div class="banner w-full relative" class:grayscale={project.deprecated}>
        <img
            src={project.bannerURL
                ? `${!project.bannerURL.startsWith('http') ? config.apiEndpoint : ""}${project.bannerURL}`
                : `/defaultbanner.png`}
            loading="lazy"
        ...
        {#if project.avatarURL}
            <img src={`${!project.avatarURL.startsWith('http') ? config.apiEndpoint : ""}${project.avatarURL}`} loading="lazy" class="cutout-element w-16 h-16 rounded-3xl absolute -bottom-8 left-4 object-cover border-8 border-surface-100 dark:border-surface-800" />
        {:else}
        ...
        {#if project}
            <div class="flex gap-3 opacity-50 items-center">
                ...
                {#if project.isServer}
                    ...
                    <span class="font-bold">{formatNumber($prefs.votes)}</span>
                {/if}
